...
export default function Widgets({newsResults, randomUsersResults}) {
  ...
  return (
    <div className="xl:w-[600px] hidden xl:inline ml-8 space-y-5">
        ...
        <div className="text-gray-700 space-y-3 bg-gray-100 rounded-lg py-3 w-[90%] xl:w-[75%]">
          <h4 className="font-bold text px-4">Whats happening ?</h4>
          <AnimatePresence>
					...
					<motion.div key={randomUser.login.username} initial={{opacity: 0 }} animate={{opacity: 1 }} exit={{opacity: 0 } } transition={{duration: 1 }}>
						<div key={randomUser.login.username} className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-200 transition duration-500 ease-out">
							...
							<div className="truncate ml-4 leading-5">
								...
								<button className="ml-auto text-black rounded-full text-sm px-3.5 py-1.5 font-bold">Follow</button>
						</div>
					</motion.div>	
				))}
			</AnimatePresence>
            <button onClick={()=>setRandomUserNum(articleNum + 3)} className="text-blue-300 pl-4 pb-3 hover:text-blue-400">Show more</button>
        </div>
